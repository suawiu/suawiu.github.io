...
그러나 이 실험에서 의미 있는 신호를 뽑아내기에는 근본적인 한계가 있었습니다. 이론적으로 bunching 신호의 크기는 $g^{(2)}(0) - 1 \propto \tau_c / \Delta t$로 주어지는데, 실험에 사용한 빛의 coherence time $\tau_c$가 사용 가능한 가장 작은 bin width($\Delta t = 0.5\,\text{ns}$)보다도 훨씬 짧았습니다. 즉, bin 내에서 bunching 신호가 완전히 평균화되어 버려 유효한 상관 신호를 검출하기 어려운 상황이었습니다. 이 문제를 해결하려면 더 좁은 linewidth를 가진 광원을 사용하거나, 더 빠른 시간 분해능의 검출기를 도입하는 것이 필요합니다.
